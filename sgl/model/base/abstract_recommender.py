...
def _create_logger(config, data_name):
    ...
    log_dir = os.path.join("log", data_name, datetime.now().strftime("%Y%m%d-%H%M%S"))
...
class AbstractRecommender(object):
    ...
    def __init__(self, config):
        ...
        self.logger, log_dir = self._create_logger(config, self.dataset)
    ...
    def _create_logger(self, config, dataset):
        ...
        log_dir = os.path.join(config.root_dir + "log", data_name, run_id)
        logger_name = os.path.join(log_dir, "log.log")
        logger = Logger(logger_name)
