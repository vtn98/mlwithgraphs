...
class SGL(AbstractRecommender):
    ...
    def train_model(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            total_loss = total_loss.item() / self.num_ratings
            bpr_loss = total_bpr_loss.item() / self.num_ratings
            reg_loss = total_reg_loss.item() / self.num_ratings
            ssl_loss = (total_loss - bpr_loss - reg_loss) / self.num_ratings
            ...
            self.tensorboard_writer.add_scalar('loss', total_loss, global_step=epoch)
            self.tensorboard_writer.add_scalar('bpr_loss', bpr_loss, global_step=epoch)
            self.tensorboard_writer.add_scalar('ssl_loss', ssl_loss, global_step=epoch)
            self.tensorboard_writer.add_scalar('reg_loss', reg_loss, global_step=epoch)


            if epoch % self.verbose == 0 and epoch > self.config['start_testing_epoch']:
                result, flag, result_dict = self.evaluate_model()
                self.logger.info("epoch %d:\t%s" % (epoch, result))
                for metric, value in result_dict.items():
                    self.tensorboard_writer.add_scalar(metric, value, global_step=epoch)
                if flag:
    ...
    def evaluate_model(self):
        flag = False
        self.lightgcn.eval()
        current_result, buf, current_result_dict = self.evaluator.evaluate(self)
        if self.best_result[1] < current_result[1]:
            self.best_result = current_result
            flag = True
        return buf, flag, current_result_dict
