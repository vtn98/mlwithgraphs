...
class Evaluator(object):
    ...
    * **Second**, this class and its evaluation metrics can automatically fit
    ...
    def evaluate(self, model, test_users=None):
        ...
        metric_show = [[("%s@" % re_metric_dict[metric] + str(k)) for k in self.top_show]
                        for metric in self.metrics]
        metric_show = reduce(lambda x, y: x + y, metric_show)  # flatten the list
        final_result_dict = dict(zip(metric_show, final_result))
        return final_result, buf, final_result_dict
